...
fn ind(x: f64) -> f64 { if x > 0.0 { 1. } else { 0. } } 

fn relu(x: f64) -> f64 { if x > 0.0 { x } else { 0. } } 
...
fn main() -> Result<(), CryptoAPIError> {
    ...
    let enc_bas = Encoder::new(0., 5., 3, 2)?;
    ...
    let enc_cmp = Encoder::new(0.0, 1.0, 1, 4).unwrap();
    ...
    */
    ...
    let x: Vec<f64> = vec![3.13, 1.88, 1.25];
    ...
    let minfun = |x_i:&VectorLWE, x_j:&VectorLWE| -> (VectorLWE, Vec<VectorLWE>) {
        ...
        let pos = diff.bootstrap_nth_with_function(&bsk00, relu, &enc_bas, 0).unwrap();
        let min = x_i.sub_with_padding(&pos).unwrap(); // x_i - tmp;
        //let idx = vec![diff,cmp,pos]; // vec![cmp,1.0-cmp];
        let mut neg = cmp.opposite_nth(0).unwrap();
        neg.add_constant_dynamic_encoder_inplace(&vec![1.]).unwrap();
        let idx = vec![neg,cmp]; // vec![cmp,1.0-cmp];
        (min, idx)
    };   
    ...
    res.0.pp();
    //println!("diff {:?}", res.1[0].decrypt_decode(&sk0).unwrap());
    println!("neg {:?}", res.1[0].decrypt_decode(&sk0).unwrap());
    res.1[0].pp();
